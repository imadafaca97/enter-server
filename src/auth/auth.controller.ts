import { Body, Controller, Delete, HttpStatus, Inject, Post, Res } from '@nestjs/common';
...
export class AuthController {
    ...
    async signup(@Body() signUpDto: SignUpDTO, @Res() res : Response){
        ...
        return res.status(HttpStatus.CREATED).cookie('token',token,{
            httpOnly: true, 
    ...
    async signin(@Body() signInDto: SignInDTO, @Res() res : Response){
        ...
        return res.status(HttpStatus.OK).cookie('token',token,{
            httpOnly: true, 
            secure: process.env['NODE_ENV'] === 'production'
        }).json({
            success: true,
