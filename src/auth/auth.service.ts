...
export class AuthService  implements IAuthService{
    ...
    private async validateSignUpEmail (email: string):Promise<void>{
       ...
       if (user) throw new BadRequestException("Ese correo electrónico ya está tomado."); 
    }

    private async validatePassword(password : string, user : Users) : Promise<void>{
        const isPasswordRight = await bcrypt.compare(password,user.password);
        
        if(!isPasswordRight) throw new ForbiddenException('Las credenciales son incorrectas')  
    }
